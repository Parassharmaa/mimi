...
enum AudioDeviceCatalog {
    ...
    static func defaultOutputDeviceID() throws -> AudioDeviceID {
        ...
        guard status == noErr, deviceID != kAudioObjectUnknown else {
            throw OutputAudioCaptureError.coreAudio("read the default output device", status)
        }
        return deviceID
    }
    ...
    static func uid(for deviceID: AudioDeviceID) throws -> String {
        ...
        guard status == noErr, !(uid as String).isEmpty else {
            throw OutputAudioCaptureError.coreAudio("read the selected output device identifier", status)
        }
    ...
    private static func hasOutput(_ deviceID: AudioDeviceID) -> Bool {
        var address = AudioObjectPropertyAddress(
            mSelector: kAudioDevicePropertyStreams,
