#!/usr/bin/env python3
"""Train the v6 full-parameter preference model with exact domain balancing."""
...
from train_marian_automated_preference_adapter import load_json, load_preferences, sha256
...
def validate_contract(
    ...
    *,
    ...
    if (
        ...
        raise SystemExit("v6 experiment contract is invalid or authorizes promotion")
    if contract.get("dataset", {}).get("directory") != str(dataset_directory):
        raise SystemExit("v6 contract binds a different dataset directory")
    for name in ("manifest.json", "train.jsonl", "valid.jsonl"):
        if contract.get("dataset", {}).get("files", {}).get(name, {}).get(
            ...
            raise SystemExit(f"v6 contract dataset hash differs: {name}")
    if contract.get("parent", {}).get("path") != str(parent) or contract.get(
        "parent", {}
    ).get("model_sha256") != sha256(parent / "model.safetensors"):
        raise SystemExit("v6 contract parent binding differs")
    if contract.get("implementation", {}).get("trainer", {}).get(
        "sha256"
    ) != sha256(Path(__file__).resolve()):
        raise SystemExit("v6 contract trainer implementation hash differs")
    return contract


def domain_balanced_rows(rows: list[dict], seed: int) -> tuple[list[dict], dict]:
    by_domain: dict[str, list[dict]] = defaultdict(list)
    for row in rows:
        by_domain[str(row.get("domain", "unknown"))].append(row)
    if len(by_domain) < 2:
        raise SystemExit("domain balancing requires at least two domains")
    ...
    for domain, values in sorted(by_domain.items()):
        ordered = sorted(
            ...
            key=lambda row: hashlib.sha256(
                f"{seed}\0{domain}\0{row['source_id']}".encode("utf-8")
            ).digest(),
        )
        for index in range(maximum):
            balanced.append(ordered[index % len(ordered)])
        repeat_counts[domain] = maximum
    if len(Counter(str(row.get("domain", "unknown")) for row in balanced)) != len(
        by_domain
    ):
        raise AssertionError("balanced domain construction lost a domain")
    return balanced, {
        "method": (
            "deterministic SHA-256 order within domain, cyclic repetition to the "
            "largest-domain count, then seeded DataLoader shuffle"
        ),
        "original_counts": dict(
            sorted(Counter(str(row.get("domain", "unknown")) for row in rows).items())
        ),
        "balanced_counts": repeat_counts,
...
def main() -> None:
    ...
    if contract.get("training") != actual_hyperparameters:
        raise SystemExit("training arguments differ from the preregistered v6 recipe")
    train_rows, valid_rows, dataset_manifest = load_preferences(
        args.preference_directory, args.direction
    )
    balanced_rows, balance = domain_balanced_rows(train_rows, args.seed)
    ...
    manifest = {
        ...
        "operation": "domain-balanced automated-consensus full-parameter preference update",
        ...
        "objective": {
            "loss": (
                "reference-relative pairwise preference + chosen SFT + "
                "sum-squared displacement to the frozen parent"
            ),
            "selection": (
